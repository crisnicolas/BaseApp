codeunit 9170 "Conf./Personalization Mgt."
{
    ...
    var
        NavAppRecord: Record "NAV App";
    begin
        if IsNullGuid(AppId) then
            exit(UserCreatedAppNameTxt);

        if NavAppRecord.ReadPermission then begin
            NavAppRecord.SetRange(ID, AppId);
            if NavAppRecord.FindFirst() then
                AppName := NavAppRecord.Name;
        end;
    end;
    ...
    begin
        ...
        BaseUrl := DelChr(BaseUrl, '>', '/'); // Our query parameter is going to add the forward slash

        UrlQueryString := StrSubstNo('/?%1&%2',
            UrlConfigureParameterTxt,
            StrSubstNo(UrlProfileParameterTxt, DotNetUri.EscapeDataString(AllProfile."Profile ID"))
            );

        exit(BaseUrl + UrlQueryString);
    end;
